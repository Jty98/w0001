            ...
            children: [
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Icon(Icons.apartment_rounded,
                              color: cs.primary, size: context.rsi(26)),
                          rsH(context, 10),
                          Expanded(
                            child: Text(
                              widget.place.pname.trim().isEmpty
                                  ? '현장'
                                  : widget.place.pname.trim(),
                      ...
                      Row(
                        children: [
                          ...
                          rsH(context, 6),
                          Text(
                            '${widget.day.year}년 ${widget.day.month}월 ${widget.day.day}일 (${widget.weekdayLabel})',
              ...
              if (widget.filtered.isEmpty)
                Material(
                  ...
                  child: Padding(
                    padding: ResponsiveLayout.symmetric(context, horizontal: 18, vertical: 22),
                    child: Text(
                ...
                for (var i = 0; i < widget.filtered.length; i++) ...[
                  ...
                  AssignmentInstructionDetailCard.fromPlaceWorkerRow(
                    workerTitle: _placeWorkerDisplayNameForHid(
                      widget.filtered[i].hid,
                      namesByHid: namesByHid,
                      viewerHid: widget.workerHid,
                      viewerUnameTrimmed: viewerUnameTrimmed,
                    ),
                    workrole: widget.filtered[i].workrole.trim(),
                    instructionBlocks: resolveAssignmentInstructionBlocks(
                      instructionBlocks: widget.filtered[i].instructionBlocks,
                    ),
                    slotLabelAboveWork: widget.filtered.length > 1
                        ? (widget.workerHid != null
                            ? '나의 배정 · ${i + 1}/${widget.filtered.length}'
                            : '투입 · ${i + 1}/${widget.filtered.length}')
                        : null,
                  ),
                ],
              ],
                ...
                SizedBox(
                  ...
                  child: snap.connectionState != ConnectionState.done
                      ? Skeletonizer(
                          enabled: true,
                          child: Padding(
                            padding: ResponsiveLayout.symmetric(context, horizontal: 8),
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                ...
                if (anyDot)
                  Padding(
                    ...
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Icon(Icons.circle, size: context.rsi(6), color: cs.tertiary),
                        rsH(context, 5),
                        Text(
                          '작업 투입일',
                          style: tt.labelSmall?.copyWith(
