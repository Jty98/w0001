...
String _sanitizeFileName(String name) {
  ...
  if (s.length > 160) {
    ...
    final base = ext.isNotEmpty ? s.substring(0, s.length - ext.length) : s;
...
Future<void> sharePlacePhotoOriginalEntries(
  ...
  if (list.isEmpty) {
    tip('공유 가능한 문서가 없습니다. (서버에 등록된 항목만 가능)');
    return;
  }
  ...
  try {
    for (var i = 0; i < list.length; i++) {
      ...
      final rawName = (fromHeader != null && fromHeader.trim().isNotEmpty)
