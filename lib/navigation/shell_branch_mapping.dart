...
/// 0: 대시보드, 1: 현장, 2: 작업지시(관리자) / 지도(작업자), 3: 캘린더, 4: 인건비, 5: 프로필(루트), 6: 설정/더보기
/// 관리자: 상황판 → 현장 관리 → 작업지시 → 캘린더 → 인건비 → 설정
const List<int> operatorShellBranchOrder = [0, 1, 2, 3, 4, 6];

/// 작업자: 대시보드 → 내 일정 → 현장 관리 → 지도 → 더보기
const List<int> workerShellBranchOrder = [0, 3, 1, 2, 6];

int shellIndexToDisplayIndex(int shellIndex, bool workerLayout) {
  final order =
      workerLayout ? workerShellBranchOrder : operatorShellBranchOrder;
