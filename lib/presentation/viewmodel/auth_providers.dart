...
class AuthSessionNotifier extends Notifier<AsyncValue<UserRead?>> {
  ...
  Future<void> loadCurrentUser({bool awaitWarmUp = false}) async {
    ...
    if (previousUser != null &&
        (previousUser.uid != user.uid || previousUser.role != user.role)) {
      clearAllUserProviders(ref.container);
    }

    final container = ref.container;
    if (awaitWarmUp && !user.isPendingApproval) {
  ...
  /// 가입 응답 등으로 이미 사용자 정보가 있을 때 세션을 채운다.
  void adoptAuthenticatedUser(UserRead user) {
    final previousUser = state.asData?.value;
